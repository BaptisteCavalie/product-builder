...
| 2026-09-13 | **Vérifier les 15 ressources du catalogue `ui-resources`** — leurs URLs n'ont pas pu être ouvertes depuis la session d'écriture (egress refusé). Une ressource morte ou dérivée se RETIRE du catalogue ; retirer aussi celles qui ne servent jamais. | `product-builder/skills/ui-resources/SKILL.md` — ouvrir chaque URL, corriger la description si elle a dérivé, supprimer l'entrée sinon (même commit, gate `check-doctrine.sh`). |
